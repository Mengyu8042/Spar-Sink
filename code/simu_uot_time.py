...
eta_list = np.array([0.4, 0.4, 0.45, 0.77])
d = 10
nloop = 10
...
for j in range(len(n_list)):
    ...
    for k in range(len(eps_list)):
        ...
        G = ot.unbalanced.sinkhorn_unbalanced(a, b, M, epsilon, reg_kl, verbose=True)
        ...
        for i in range(nloop):
            ...
            R, A = unifNys(K, s)
            WFR_nys = nys_sinkhorn_unbalanced(a, b, M, R, A, epsilon, reg_kl, stopThr=1e-4)
            
            if np.isnan(WFR_nys):
                K_nys = np.dot(np.dot(R, A), R.T)
                WFR_nys = nys_sinkhorn_unbalanced2(a, b, M, K_nys, epsilon, reg_kl, stopThr=1e-4)
            ...
            WFR_spar = spar_sinkhorn_unbalanced(a, b, M, K, s, epsilon, reg_kl, stopThr=1e-4)
